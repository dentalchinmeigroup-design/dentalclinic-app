...
def connect_to_google_sheets():
    ...
    try:
        ...
        # 開啟試算表 (請確認您的 Google 試算表檔案名稱為這個)
        spreadsheet_name = "dental_assessment_data" 
        sh = client.open(spreadsheet_name)
        return sh
    except Exception as e:
        st.error(f"❌ 連線失敗！\n1. 請確認 Google 試算表檔案名稱是否已改為 '{spreadsheet_name}'？\n2. 機器人信箱是否已設為編輯者？\n錯誤訊息: {e}")
        st.stop()
    ...
    if st.button("🚀 提交完整考核表", type="primary", use_container_width=True):
        if not name:
            ...
            with st.spinner("正在連線 Google Sheets 並寫入資料..."):
                try:
                    # 1. 取得工作表 (自動建立或讀取 Assessment_Data)
                    try:
